...
function createGitWithProgress(
  ...
  return simpleGit(process.cwd()).outputHandler((_cmd, _stdout, stderr) => {
    ...
    const onData = (chunk: Buffer | string) => {
      ...
      let m = GIT_CLONE_PERCENT.exec(s);
      while (m !== null) {
        last = Number(m[1]);
        m = GIT_CLONE_PERCENT.exec(s);
      }
...
export async function clone(
  ...
  const git = opts?.onProgress
    ? createGitWithProgress(opts.onProgress)
    : defaultGit;
  await withRetry(() => git.clone(gitURL, path), {
    maxAttempts: 3,
    delayMs: 2000,
  });
